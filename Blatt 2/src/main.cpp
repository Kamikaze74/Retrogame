...
typedef enum { NONE = 0, EVEN = 2, ODD = 3 } Parity;
...
int main(void) {
    // Initialisierung: 9600 Baud, 8 Datenbits, Keine Parität, 1 Stopbit
    initUsart(19200, 8, NONE, 1);

    usartPutString("USART Initialisiert...\r\n");

    while (1) {
        int received = usartGetchar();
        
        // Echo-Funktion: Wenn ein Zeichen empfangen wurde, schicke es zurück
        if (received != -1) {
            usartPutchar('[');
            usartPutchar((unsigned char)received);
            usartPutchar(']');
        }
    }
